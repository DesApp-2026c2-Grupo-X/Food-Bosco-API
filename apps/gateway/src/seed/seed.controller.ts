...
export class SeedController {
  ...
  async seed(): Promise<{
    ...
    const context = {
      internalToken: env.internalApiToken,
    }

    const commerceResult =
      await this.commerce.post<CommerceSeedResult>(
        '/v1/seed',
        { context },
      )
    ...
    const authResult =
      await this.auth.post<AuthSeedResult>(
        '/v1/seed',
        {
          context,
          body: { branchId },
    ...
    const rider = authResult.users.find(
    ...
    if (rider) {
      const deliveryResult =
        await this.delivery.post<{ summary: Raw }>(
          '/v1/seed',
          {
            context,
            body: {
